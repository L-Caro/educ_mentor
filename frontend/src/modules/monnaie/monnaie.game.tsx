...
} from 'src/modules/monnaie/monnaie.type.ts';
...
import {
  ...
  getMonnaieImageUrl,
  parseMoneyInput,
} from './constants/denominations.ts';
...
function questionSummary(question: MonnaieQuestion): string {
  switch (question.type) {
    case 'reconnaitre':
      return `${(question.coins ?? []).length} pièce(s)/billet(s)`;
    case 'total':
      return `Total de ${(question.prices ?? []).map(formatCents).join(' + ')}`;
    case 'rendre':
      return `${formatCents(question.price ?? 0)} → tu donnes ${formatCents(question.payment ?? 0)}`;
  }
}
      ...
      return (
        <div className="MonnaieGame__receipt">
          <div className="MonnaieGame__receipt-row">
            ...
            <span className="MonnaieGame__receipt-value">
              {formatCents(question.price ?? 0)}
            </span>
          </div>
          <div className="MonnaieGame__receipt-row">
            <span className="MonnaieGame__receipt-label">Tu donnes</span>
            <span className="MonnaieGame__receipt-value">
              {formatCents(question.payment ?? 0)}
            </span>
          </div>
        </div>
      );
...
export const monnaieGameSpec: GameModuleSpec<
  MonnaieSessionResponse,
  MonnaieQuestion
> = {
  loadSession: (setup) => {
    ...
    if (!exerciseType)
      return Promise.reject(new Error("Type d'exercice manquant."));
    return store
      .dispatch(
        monnaieApi.endpoints.startMonnaieSession.initiate({
          exerciseType,
  ...
  free: {
    ...
    isCorrect: (question, given) =>
      typeof given === 'number' && given !== -1 && given === question.answer,
    inputProps: {
      inputMode: 'decimal',
      suffix: '€',
      maxLength: 8,
  ...
  recordAnswer: (sessionId, question, correct) =>
    store
      .dispatch(
        monnaieApi.endpoints.recordMonnaieAnswer.initiate({
          sessionId,
  ...
  completeSession: (sessionId, correctAnswers, totalQuestions) =>
    store
      .dispatch(
        monnaieApi.endpoints.completeMonnaieSession.initiate({
          sessionId,
          correctAnswers,
          totalQuestions,
        }),
  ...
  buildResultEntry: (question, given, correct, timeout) => {
    ...
    return {
      ...
      given:
        cents === null || Number.isNaN(cents) || cents < 0
          ? null
          : formatCents(cents),
