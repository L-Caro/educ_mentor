...
export default function MahjongGame() {
  ...
  /** Dit qu'un melange n'a rien donne, le temps de l'annoncer. Voir `melanger`. */
  const [melangeImpossible, setMelangeImpossible] = useState(false);
   ...
   *
   * Sans limite, volontairement. Un melange limite a trois est une regle de jeu de
   * competition ; ici l'enjeu est qu'une enfant de sept ans ne reste pas bloquee devant
   * un plateau qu'elle ne sait plus ouvrir. Elle ne compte pas comme un essai non plus :
   * rebattre n'est pas se tromper.
   *
   * Le nouveau plateau est solvable par construction (voir `melangerPlateau`) : c'est
   * tout l'interet de repasser par le generateur plutot que de permuter les faces.
   */
  const melanger = useCallback(() => {
    if (enPauseRef.current) return;
    const rebattu = melangerPlateau(cases);
    if (!rebattu) {
      setMelangeImpossible(true);
      setTimeout(() => setMelangeImpossible(false), DELAI_BLOCAGE_MS);
      return;
    }
    setCases(rebattu);
    setSelectionId(null);
    setBlocage(null);
    setMelangeImpossible(false);
  }, [cases]);
  ...
  return (
    <div className="MahjongGame">
      <p className="MahjongGame__compteur">
      ...
      <p className="MahjongGame__blocage" role="status">
        ...
        {melangeImpossible && 'Je n’ai pas trouvé de nouveau mélange. Le plateau reste tel quel.'}
      </p>

      <div className="MahjongGame__actions">
        <Button variant="ghost" size="sm" onClick={melanger}>
          🔀 Mélanger
        </Button>
      </div>
